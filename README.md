...
**Latest Update:** The app now features a full-featured image editor with drawing, shapes, mosaic tools, and transform capabilities. Image grid splitting allows exporting images into grid tiles with selective export. AI painting has been significantly enhanced with a prompt descriptor selector, multi-image preview, streaming response processing, foreground service for reliable generation, and notification-based session navigation. The theme system has been upgraded with dynamic themes, a translucent theme option, and full immersive layout support. Additional improvements include in-app update checking, cache management, and glassmorphism UI elements.
...
**最新更新：** 应用现已拥有功能完善的图片编辑器，支持画笔涂鸦、形状绘制、马赛克涂抹以及移动/缩放/旋转等变换操作。新增图片网格切分功能，支持选择性导出切片。AI 绘画功能大幅增强，引入描述词选择器（多场景切换）、多图预览、流式响应处理、前台服务保障生成以及通知跳转回指定会话。主题系统全面升级，新增动态主题、清透主题和沉浸式布局。此外还新增了应用内更新检查、缓存管理和玻璃拟态 UI 元素等功能。
