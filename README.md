...
**Latest Update:** The AI painting module has undergone a core architecture refactoring, introducing a global `GenerationTaskManager` that decouples generation tasks from the ViewModel, enabling multi-task parallel generation and persistent background execution. The description picker now outputs selections grouped by category with auto-prefixed labels for better readability. Message bubbles now display precise timestamps and generation parameter tags (model, aspect ratio, resolution). The bottom input bar layout has been fully restructured with buttons moved inside the input container for a more compact look, and a new reference image placeholder card improves the image-adding workflow. The Enhance Prompt feature has been removed to simplify the user flow. The session list now shows a generation-in-progress indicator, and stopping generation requires a confirmation dialog. Version upgraded to 1.8.
...
**最新更新：** AI 绘画模块进行了核心架构重构，引入全局生成任务管理器（`GenerationTaskManager`），将生成任务与 ViewModel 解耦，支持多任务并行生成和后台持续运行。描述词选择器新增分类分组输出，按类别自动拼接前缀提升可读性。消息气泡新增精确时间戳和生成参数标签（模型、比例、分辨率）。底部输入栏布局全面重构，按钮移入输入框内部使外观更紧凑，并引入参考图占位卡片优化图片添加交互。移除了提示词优化（Enhance Prompt）功能以简化操作流程。会话列表新增生成中状态指示器，停止生成操作增加二次确认。版本升级至 1.8。
