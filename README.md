...
**Latest Update:** Integrated GPT Image API (`gpt-image-2` model) with support for image generation and multi-reference-image editing. A streaming response processor (`AndroidGptImageResponseProcessor`) keeps peak memory at ~200KB, avoiding OOM on large images. Added a Session Stats screen with analytics (prompt count, success rate, average generation time, draft hints). Introduced an Image Compare Preview component for before/after comparison. Optimized the regeneration flow to read parameters synchronously from UI state, eliminating race conditions. Aspect ratio selection now dynamically maps to the closest GPT image size. Removed the Enhance Prompt backend logic entirely. Refined message action button styles (rounded-rectangle with outlined icons). Version upgraded to 1.9.
...
**最新更新：** 集成 GPT Image API（`gpt-image-2` 模型），支持图片生成与多参考图编辑功能。引入流式响应处理器（`AndroidGptImageResponseProcessor`），峰值内存仅 ~200KB，避免大图 OOM。新增会话统计界面，提供提示词数量、成功率、平均耗时、草稿建议等数据洞察。新增图片对比预览组件（ImageComparePreview）。优化重新生成流程，改为从 UI 状态同步读取参数，消除竞态问题。尺寸选择逻辑升级，可根据比例动态匹配最接近的 GPT 图片尺寸。彻底移除提示词优化（Enhance Prompt）后端逻辑。消息操作按钮样式优化为圆角矩形 + 轮廓图标。版本升级至 1.10。
...
- ✏️ **画笔涂鸦**：支持自由画笔涂鸦，可调节颜色和粗细
- 🔷 **形状绘制**：支持绘制矩形、圆形等基础形状
- 🟫 **马赛克涂抹**：支持对图片局部进行马赛克处理
- 🔄 **图形变换**：支持移动、缩放和旋转已绘制的图形
- 💾 **保存涂鸦**：支持保存涂鸦和形状到图片
...
- 🧠 **GPT Image 支持**：集成 GPT Image API（`gpt-image-2`），支持图片生成与基于多参考图的编辑功能，最高支持 16 张参考图
- 🖼️ **参考图支持**：支持上传参考图片，辅助 AI 生成更精准的结果
- 💬 **会话管理**：支持创建、切换和删除多个绘画会话
- 🎯 **多模型选择**：支持 Gemini 2.5 Flash、Gemini 3 Pro 和 GPT Image 2 三种模型
- 📐 **灵活配置**：可自定义宽高比（1:1、16:9、9:16）和分辨率，GPT 模型支持独立的尺寸、质量和输出格式配置
- 📊 **会话统计**：内置会话数据分析面板，展示提示词数量、生成次数、成功率、平均耗时、草稿建议等洞察数据
- � **图片对比预览**：新增图片对比预览组件，支持参考图与生成结果的前后对比
- �📝 **历史记录**：保存所有生成记录，支持查看和管理
- 💾 **图片保存**：支持将生成的图片保存到本地相册，根据文件格式自动设置正确的 MIME 类型
- 🔄 **图片编辑**：支持旋转、镜像等基础编辑功能
- 🏷️ **描述词选择器**：内置多场景描述词选择器，支持分类分组输出并自动拼接前缀
- 🖼️ **多图预览**：支持预览 AI 一次性生成的多张图片
- 📡 **流式处理**：引入图片响应流式处理器，GPT 模型采用流式 Base64 解码，峰值内存仅 ~200KB
- 🔔 **前台服务**：通过前台服务保障图片生成，支持通知跳转回指定会话
- 🗜️ **图片压缩**：自动压缩参考图，优化内存占用
- 🔄 **全局任务管理**：生成任务与 ViewModel 解耦，支持多任务并行和后台持续运行

### 应用设置

- 🎨 **主题切换**：支持浅色、深色和跟随系统三种主题模式
- 🌈 **动态主题**：集成 MaterialTheme 动态主题系统
- 🪟 **清透主题**：新增清透主题风格
- 📱 **沉浸式布局**：全面启用沉浸式布局体验
- 🔄 **应用内更新**：支持应用内检查更新
- 🗑️ **缓存管理**：新增缓存管理功能，可查看和清理缓存
- 🌐 **多语言支持**：内置中英文（简体中文/英文）切换，所有 UI 文案均通过统一国际化资源管理
- 💾 **配置持久化**：所有配置自动保存，重启后恢复
- 🎬 **流畅动画**：Activity 切换支持滑动动画效果
- 🪄 **玻璃拟态 UI**：引入玻璃拟态按钮等现代化 UI 元素
...
- **网络请求**：Ktor Client
- **AI 模型**：Google Gemini API、OpenAI GPT Image API
...
- **Gemini API Key**：使用 AI 绘画 Gemini 模型需要（可选）
- **GPT Image API Key**：使用 AI 绘画 GPT Image 模型需要（可选）
...
1. **配置 API**：
   - 首次使用需要配置 API Key
   - 在 AI 绘画界面点击设置按钮
   - 输入 Gemini API Key 或 GPT Image API Key 和 API URL（可选）
...
3. **生成图片**：
   - 在输入框中输入描述文字
   - 可选：点击图片图标上传参考图（GPT 模型最多支持 16 张）
   - 选择模型（Gemini 或 GPT Image）、宽高比和分辨率
   - GPT 模型可额外配置图片尺寸、质量和输出格式
   - 点击发送按钮开始生成

4. **查看统计**：
   - 点击顶部标题区域可切换至会话统计面板
   - 查看提示词数量、生成次数、成功率、平均耗时等数据
   - 支持查看草稿建议和最近提示词记录
