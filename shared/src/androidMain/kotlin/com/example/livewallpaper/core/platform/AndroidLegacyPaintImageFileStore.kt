...
/** Android private-file implementation used while preserving legacy embedded images. */
class AndroidLegacyPaintImageFileStore(
    ...
    override suspend fun isReadable(identifier: String): Boolean = withContext(dispatchers.io) {
        if (identifier.startsWith("content://")) {
            runCatching { context.contentResolver.openInputStream(Uri.parse(identifier))?.use { true } ?: false }
                .getOrDefault(false)
        } else {
    ...
    override suspend fun writeLegacyImage(
        ...
        runCatching {
            val directory = File(context.filesDir, "aipaint/$sessionId/legacy").apply { mkdirs() }
